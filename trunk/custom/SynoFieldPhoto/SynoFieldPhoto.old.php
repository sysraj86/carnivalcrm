        ...
        if(     
            ...
            foreach ($focus->field_name_map as $key => $fieldDef) {
                if (
                    ...
                    echo "alert(' $focus->picture');";
                    echo "</script>";
                    $t = $focus->picture;
                    $picture_path = 'custom/SynoFieldPhoto/phpThumb/images/new_'.$focus->picture;//      $focus->module_dir.'_'.$focus->field_name_map[$key]['name'].'_'.$focus->date_entered.'.jpg';
        ...
        elseif(
            ...
            foreach ($focus->field_name_map as $key => $fieldDef) {
                if (
                    ...
                    $picture_path = 'custom/SynoFieldPhoto/phpThumb/images/'.$focus->picture;//$focus->module_dir.'_'.$focus->field_name_map[$key]['name'].'_'.$focus->date_entered.'.jpg';
                    if(is_file( $picture_path )){
                        $GLOBALS['log']->debug(__FUNCTION__.' '.$event.' Remove '.$picture_path); 
                        @unlink($picture_path);
                    }
            ...
            foreach ($focus->field_name_map as $key => $fieldDef) {
                if (
                    ...
                    if(!empty($focus->$picture_field_name)){
                        ...
                        $old_picture_path = 'custom/SynoFieldPhoto/phpThumb/images/'.$focus->module_dir.'_'.$focus->field_name_map[$key]['name'].'_'.$time.'.jpg';
                        $new_picture_path = 'custom/SynoFieldPhoto/phpThumb/images/new_'.$focus->module_dir.'_'.$focus->field_name_map[$key]['name'].'_'.$time.'.jpg';
