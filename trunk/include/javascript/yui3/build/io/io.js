...
YUI.add('io-base', function (Y) {
    var E_START = 'io:start', E_COMPLETE = 'io:complete', E_SUCCESS = 'io:success', E_FAILURE = 'io:failure', E_END = 'io:end', transactionId = 0, _headers = {'X-Requested-With':'XMLHttpRequest'}, _timeout = {}, w = Y.config.win;
    ...
    function _ioComplete(o, c) {
        var evt, r = o.status ? {status:0, statusText:o.status} : o.c;
        c.on = c.on || {};
    ...
    function _startTimeout(o, timeout) {
        _timeout[o.id] = w.setTimeout(function () {
            _ioCancel(o, 'timeout');
        }, timeout);
    }
...
YUI.add('io-form', function (Y) {
    Y.mix(Y.io, {_serialize:function (c, s) {
        var eUC = encodeURIComponent, data = [], useDf = c.useDisabled || false, item = 0, id = (typeof c.id === 'string') ? c.id : c.id.getAttribute('id'), e, f, n, v, d, i, il, j, jl, o;
        if (!id) {
            id = Y.guid('io:');
...
YUI.add('io-xdr', function (Y) {
    var E_XDR_READY = 'io:xdrReady', _fn = {}, _rS = {};

    function _swf(uri, yid) {
        var o = '<object id="yuiIoSwf" type="application/x-shockwave-flash" data="' +
            uri + '" width="0" height="0">' + '<param name="movie" value="' + uri + '">' + '<param name="FlashVars" value="yid=' + yid + '">' + '<param name="allowScriptAccess" value="always">' + '</object>', c = document.createElement('div');
        document.body.appendChild(c);
        c.innerHTML = o;
    }

    function _xdr(o, c) {
        o.c.onprogress = function () {
            _rS[o.id] = 3;
        }
        o.c.onload = function () {
            _rS[o.id] = 4;
    ...
    Y.mix(Y.io, {_transport:{}, xdr:function (uri, o, c) {
        if (c.on && c.xdr.use === 'flash') {
            _fn[o.id] = {on:c.on, context:c.context, arguments:c.arguments};
            c.context = null;
            c.form = null;
            o.c.send(uri, c, o.id);
        }
        ...
        var m, fn, isFlash = c.xdr.use === 'flash' ? true : false, isXML = c.xdr.dataType === 'xml' ? true : false;
        c.on = c.on || {};
        if (isFlash) {
            m = _fn || {};
...
YUI.add('io-upload-iframe', function (Y) {
    ...
    function _handle(o, c) {
        var d = Y.one('#ioupload' + o.id).get('contentWindow.document'), b = d.one('body'), xml = (d._node.nodeType === 9), p;
        if (c.timeout) {
            _clearTimeout(o.id);
        }
        if (b) {
    ...
    function _startTimeout(o, c) {
        Y.io._timeout[o.id] = w.setTimeout(function () {
    ...
    Y.mix(Y.io, {_upload:function (o, uri, c) {
        var f = (typeof c.form.id === 'string') ? Y.config.doc.getElementById(c.form.id) : c.form.id, fields, attr = {action:f.getAttribute('action'), target:f.getAttribute('target')};
        _create(o, c);
...
YUI.add('io-queue', function (Y) {
    var _q = new Y.Queue(), _e, _activeId, _qState = 1;
    ...
    ;
    _e = Y.on('io:complete', function (id) {
        _next(id);
    }, Y.io);
    _queue.size = _size;
    _queue.start = _start;
    ...
    Y.mix(Y.io, {queue:_queue}, true);
}, '3.0.0', {requires:['io-base', 'queue-promote']});
YUI.add('io', function (Y) {
}, '3.0.0', {use:['io-base', 'io-form', 'io-xdr', 'io-upload-iframe', 'io-queue']});