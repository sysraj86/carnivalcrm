...
YUI.add('profiler', function (Y) {
    var container = {}, report = {}, stopwatches = {}, WATCH_STARTED = 0, WATCH_STOPPED = 1, WATCH_PAUSED = 2, L = Y.Lang;

    function createReport(name) {
        report[name] = {calls:0, max:0, min:0, avg:0, points:[]};
        return report[name];
    }

    function saveDataPoint(name, duration) {
        var functionData = report[name];
        if (!functionData) {
    ...
    Y.Profiler = {clear:function (name) {
        if (L.isString(name)) {
            delete report[name];
        ...
        var newMethod = function () {
            var start = new Date(), retval = method.apply(this, arguments), stop = new Date();
        ...
        var now = new Date(), stopwatch = stopwatches[name];
        ...
        if (container[name]) {
            ...
            if (!stopwatches[name]) {
                stopwatches[name] = {state:WATCH_STOPPED, start:0, total:0};
            }
    ...
    }, registerConstructor:function (name, owner) {
        this.registerFunction(name, owner, true);
    }, registerFunction:function (name, owner, registerPrototype) {
        var funcName = (name.indexOf(".") > -1 ? name.substring(name.lastIndexOf(".") + 1) : name), method, prototype;
        if (!L.isObject(owner)) {
    ...
    }, unregisterConstructor:function (name) {
        if (L.isFunction(container[name])) {
            this.unregisterFunction(name, true);
        }
    }, unregisterFunction:function (name, unregisterPrototype) {
        if (L.isFunction(container[name])) {
            if (unregisterPrototype) {
                this.unregisterObject(name + ".prototype", container[name].prototype);
            }
            var owner = container[name].__yuiOwner, funcName = container[name].__yuiFuncName;
