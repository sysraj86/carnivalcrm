...
YUI.add('slider', function (Y) {
    ...
    Y.extend(Slider, Y.Widget, {_key:null, _factor:1, _railSize:null, _thumbSize:null, _thumbOffset:0, _stall:false, _disabled:false, initializer:function () {
        ...
        var cb = this.get(CONTENT_BOX), rail = this.get(RAIL);
        if (!rail) {
            rail = cb.appendChild(Y.Node.create('<div class="' + C_RAIL + '"></div>'));
            this.set(RAIL, rail);
        ...
        var rail = this.get(RAIL), thumb = this.get(THUMB);
        if (thumb && !this.get(THUMB_IMAGE) && thumb.get('nodeName').toLowerCase() === 'img') {
            this.set(THUMB_IMAGE, thumb);
            this.set(THUMB, null);
            thumb = null;
        }
        if (!thumb) {
            thumb = Y.Node.create('<div class="' + C_THUMB + '"></div>');
        ...
        var thumb = this.get(THUMB), img = this.get(THUMB_IMAGE);
        if (img) {
        ...
        var ddConf = {node:this.get(THUMB), bubble:false}, conConf = {constrain2node:this.get(RAIL)};
        ...
        if (this.get('railEnabled') && !this.get(DISABLED)) {
            var dd = this._dd, xyIndex = this._key.xyIndex, xy;
        ...
        this.fire(POSITION_THUMB, {value:val, offset:this._convertValueToOffset(val)});
        this.get('boundingBox').toggleClass('');
    }, _uiSetThumbSize:function () {
        var thumb = this.get(THUMB), dim = this._key.dim, img = this.get(THUMB_IMAGE), size;
        ...
        var rail = this.get(RAIL), thumb = this.get(THUMB), img = this.get(THUMB_IMAGE), dim = this._key.dim, size = this.get(RAIL_SIZE), setxy = false;
        if (parseInt(size, 10)) {
            ...
            size = max(size | 0, parseInt(thumb.getComputedStyle(dim), 10), parseInt(rail.getComputedStyle(dim), 10));
            if (img && this._isImageLoaded(img)) {
        ...
        var gutter = this._key.xyIndex ? this.get(MIN_GUTTER) + " 0 " + this.get(MAX_GUTTER) : "0 " + this.get(MAX_GUTTER) + " 0 " + this.get(MIN_GUTTER);
        ...
        this._factor = this._railSize ? (this.get(MAX) - this.get(MIN)) / range : 1;
    }, getValue:function () {
        ...
        return isNumber(v) && (min < max ? (v >= min && v <= max) : (v >= max && v <= min));
        ...
        return isString(v) && (v === '0' || /^\d+(?:p[xtc]|%|e[mx]|in|[mc]m)$/.test(v));
        ...
        return v ? Y.get(v) || Y.Node.create('<img src="' + v + '" alt="Slider thumb">') : null;
        ...
        var before = this.get(VALUE), val = e.ddEvent[this._key.eventPageAxis] - this._offsetXY;
        ...
        var dd = this._dd, thumb = dd.get('dragNode'), hidden = thumb.ancestor(this._isDisplayNone);
        if (!hidden) {
