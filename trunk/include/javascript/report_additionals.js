...
function addFilterInputDate(row, filter) {
    ...
    row.appendChild(cell);
    Calendar.setup({inputFieldObj:new_input, buttonObj:img_element, ifFormat:cal_date_format, showsTime:false, singleClick:true, step:1, weekNumbers:false});
    var filter_row = filters_arr[filters_count_map[current_filter_id]];
    filter_row.input_field0 = new_input;
    filter_row.input_field1 = null;
}
...
function addFilterInputDateBetween(row, filter) {
    ...
    cell.appendChild(img_element);
    row.appendChild(cell);
    Calendar.setup({inputFieldObj:new_input, buttonObj:img_element, ifFormat:cal_date_format, showsTime:false, singleClick:true, weekNumbers:false, step:1});
    var cell = document.createElement('td');
    cell.setAttribute('valign', 'middle');
    var new_text = document.createTextNode(lbl_and);
    cell.appendChild(new_text);
...
function joinChecked(obj) {
    ...
    for (var index in filters_arr) {
        ...
        if (objName.indexOf("_div") > -1 && objName.substr(0, objName.indexOf("_div")) == filter_module) {
            ...
            var filter = {column_name:filter_row.module_select[filter_row.module_select.selectedIndex].value, qualifier_name:'', input_name0:'', input_name1:''};
...
function reload_columns(reload_type) {
    ...
    for (i = 0; i < visible_summary_fields.length; i++) {
        ...
        if (current_db_type != 'mysql' && (typeof(valid_groups[key]) == 'undefined' && (typeof(all_fields[key].field_def.summary_type) == 'undefined' || all_fields[key].field_def.summary_type != 'group'))) {
            continue;
        }
