<?php
    ...
    */
    ...
    $des = array();
    for ($k = $k; $k < $destination_count[$i]*($i+1); $k++) {
    if (!empty($d[$k])) {
    $des[] = $d[$k];
    }
    }
    ...
    */
    ...
    $tourProgramCountLine = count($_POST['title']);

    for ($i = 0; $i < $tourProgramCountLine; $i++) {
        $tourProgram = new TourProgram();
        $tourProgram->id = $_POST['tour_program_id'][$i];
        $tourProgram->tour_id = $return_id;
        $tourProgram->title = $_POST['title'][$i];
        $tourProgram->areas = $_POST['tour_program_areas'.($i+1)];
        if(!empty($tourProgram->areas)) {
            $tourProgram->areas = implode(',',$tourProgram->areas);  
        }
        $tourProgram->countries = $_POST['tour_program_countries'.($i+1)];
        if(!empty($tourProgram->countries)){
        ...
        if(!empty($tourProgram->location)){
            $tourProgram->location = implode(',',$tourProgram->location);  
        }
        $tourProgram->notes = $_POST['notes'][$i];
        $tourProgram->description = str_replace("\\\"", "&quot;", $_POST['description_pro'][$i]);
        $tourProgram->picture = $_POST['images'][$i];
        // begin upload image
