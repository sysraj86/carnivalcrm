        ...
        */
        ...
        public function get_list_areas_by_countries($countries, $type = 0)
        {
            ...
            if ($countries && count($countries) > 0) {
                ...
                FROM
                c_areas a 
                INNER JOIN countries_c_areas_c ac 
                ON a.id = ac.countries_92a9c_areas_idb 
                JOIN countries c 
                ON c.id = ac.countries_f060untries_ida 
                WHERE a.deleted = 0 
                AND c.deleted = 0 
                AND ac.deleted = 0 " . $where;
                $result = $db->query($query);
                if ($type == 1) {

                    while ($row = $db->fetchByAssoc($result)) {
                        $areas[$row['id']] = $row['name'].'-'.$row['country'];
                    }
