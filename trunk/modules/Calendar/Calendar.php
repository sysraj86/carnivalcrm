...
class CalendarActivity
{
  ...
  function get_freebusy_activities($user_focus, $start_date_time, $end_date_time)
  {
 	...
 	function get_activities($user_id, $show_tasks, $view_start_time, $view_end_time, $view) {
		...
		*/
		if(ACLController::checkAccess('Meetings', 'list', true )) { //$current_user->id == $user_id)) {
		/* END - SECURITY GROUPS */
			$meeting = new Meeting();
		...
		*/
		if(ACLController::checkAccess('Calls', 'list', true )) { //$current_user->id == $user_id)) {
		...
		if($show_tasks) {
			...
			*/
			if(ACLController::checkAccess('Tasks', 'list', true )) { //$current_user->id == $user_id)) {
