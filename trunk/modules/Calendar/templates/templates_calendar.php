...
<?php
	...
	function template_echo_slice_activities_shared($args) {
		...
		$calls_mod_strings = return_module_language($current_language, 'Calls');
		$meetings_mod_strings = return_module_language($current_language, 'Meetings');
		/* END - SECURITY GROUPS */
		...
		foreach($args['slice']->acts_arr[$shared_user->id] as $act) {
			...
			if($act->sugar_bean->object_name == 'Call') {
				...
				/* BEGIN - SECURITY GROUPS */ 
				if($act->sugar_bean->show_as_busy == true) {
					echo "<td class=\"monthCalBodyDayItemTd\" width=\"100%\">Busy ".
					$timedate->to_display_time($act->sugar_bean->time_start, true, false); 
					echo " (".$act->sugar_bean->duration_hours . $meetings_mod_strings['LBL_HOURS_ABBREV']." ";
					echo $act->sugar_bean->duration_minutes . $meetings_mod_strings['LBL_MINSS_ABBREV'].")";					    
					echo "</td></tr>";
				} else 
				/* END - SECURITY GROUPS */		
