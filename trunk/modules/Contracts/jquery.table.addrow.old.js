(function($){if(typeof $.fn.btnAddRow== "undefined"){
	...
	ExpandableTable.prototype.addRow=function(){
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
            ...
            // Phung Duong add code for selectlist element and Span
            var t=  newRow.find("select");
            var chuoi=t[0].className;
            var chiso=parseInt(chuoi.substring(chuoi.length,chuoi.length-1));
            var temp=chiso;
            
            chiso++;
            // cat ky tu chi so 
            chuoi=chuoi.replace(temp,"");
            // gan tiep chi so moi
            chuoi=chuoi+chiso;
            newRow.find("select").removeClass();
            newRow.find("select").addClass(chuoi);
            
            // remove style of element input text
            newRow.find("input:text").removeAttr("style");
            newRow.find("input:radio").removeAttr("style");
            newRow.find("input:button").removeAttr("style");
            newRow.find("input:image").removeAttr("style");
            
            newRow.find("textarea").removeAttr("style");
            newRow.find("span").removeAttr("style");
            
            // for span to display date format
            
            //var index=1;
            // voi the span lay ID lam can cu de nhan ban chi so
            var findSpan=  newRow.find("span"); 
            if(findSpan.length>0)
            {
                var temp1=  jQuery(findSpan[0]).attr("id");  
              //  alert(temp1)      ;
                var index=parseInt(temp1.substring(temp1.length,temp1.length-1));
                
                //alert(jQuery(findSpan[0]).attr("id"));
             //   alert(jQuery(findSpan[1]).attr("id"));
                ...
                for(var i=0;i<findSpan.length;i++)
                {
                    ...
                    temp_index=index;
                  //  alert(i);
                    var sk=  jQuery(findSpan[i]).attr("class");
                     alert(sk);
                    ///document.write(sk)  ;
                     //alert(sk);
                   //  alert(i);
                   ...
                   // alert(chuoi.lenght)l
                    ...
                    //newRow.find("span").removeClass();
                    //newRow.find("span").addClass(chuoi); 
                     jQuery(findSpan[i]).removeClass();     
                     jQuery(findSpan[i]).addClass(chuoi);     
                      
                }
               // alert(index);
               /*
                newRow.find("span").each(function (i)
                {
                    alert(jQuery(findSpan[i]).attr("id"));        
                     var sk=  jQuery(this).attr("id");
                    // alert(sk);
                   //  alert(i);
                     //alert(sk.length);
                    var chuoi=sk;
                    ...
                    // gan tiep chi so moi
                    chuoi=chuoi+index;
                    newRow.find("span").removeAttr("id");
                    newRow.find("span").attr("id",chuoi);   
...
jQuery("document").ready(function(){
	jQuery(".addrow").btnAddRow({inputBoxAutoNumber:"true", maxRow:"5"});
...
function addRow(table)
{
    ...
    var trcopy=$(tr).clone();
    ///trcopy.find("")
    $("input","select",trcopy).each(function()
    {
        var id=this.id;
        var idnew=id+""+number;
        $(this).attr("id",idnew); 
...
function deleteRow(index,table)
{
    ...
    {
        var trtmp=tr[i];
        $("input","select",trtmp).each(function()
        {
            var id=this.id;
            var idnew=id.substr(0,id.length()-1);
            idnew=idnew+""+i;
            $(this).attr("id",idnew);
        })
    }
