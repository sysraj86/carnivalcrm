(function () {
    ...
    tinymce.create('tinymce.plugins.Compat2x', {getInfo:function () {
        return{longname:'Compat2x', author:'Moxiecode Systems AB', authorurl:'http://tinymce.moxiecode.com', infourl:'http://wiki.moxiecode.com/index.php/TinyMCE:Plugins/compat2x', version:tinyMCE.majorVersion + "." + tinyMCE.minorVersion};
    }});
    (function () {
        tinymce.extend(tinyMCE, {addToLang:function (p, l) {
            each(l, function (v, k) {
    ...
    (function () {
        ...
        tinymce.extend(tinyMCE, {getParam:function (n, dv) {
            ...
            if (doc == document)this.loadCSS(u); else new tinymce.dom.DOMUtils(doc).loadCSS(u);
        }, log:function () {
            console.debug.apply(console, arguments);
        }, getLang:function (n, dv) {
            var v = EditorManager.activeEditor.getLang(n.replace(/^lang_/g, ''), dv);
            if (/^[0-9\-.]+$/g.test(v))return parseInt(v);
            return v;
            ...
            if (na)na = new RegExp('^(' + na.toUpperCase().replace(/,/g, '|') + ')$', 'g');
            ...
            return ed.controlManager.createButton(cn, {title:lang, command:c, ui:u, value:v, scope:this, 'class':'compat', image:img});
            ...
            function PluginWrapper(ed) {
                ...
                ed.onInit.add(function () {
                    ...
                    if (f.initInstance)f.initInstance(ed);
                ...
                this.createControl = function (n) {
                    ...
                    if (f.getControlHTML)return f.getControlHTML(n);
                    return null;
                };
                this.execCommand = function (cmd, ui, val) {
                    if (f.execCommand)return f.execCommand(ed.id, ed.getBody(), cmd, ui, val);
                    return false;
                };
            }
        ...
        tinymce.extend(tinymce.Editor.prototype, {getFocusElement:function () {
        ...
        tinymce.extend(tinymce.dom.Selection.prototype, {getSelectedText:function () {
            ...
            if (!is(collapse))collapse = true;
            if (collapse) {
                if (!is(to_start))to_start = true;
                t.collapse(to_start);
            }
        }});
    }).call(this);
