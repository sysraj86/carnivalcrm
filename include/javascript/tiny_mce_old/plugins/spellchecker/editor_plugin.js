(function () {
    ...
    tinymce.create('tinymce.plugins.SpellcheckerPlugin', {getInfo:function () {
        ...
        if (n == 'spellchecker') {
            ...
            c.onRenderMenu.add(function (c, m) {
                ...
                each(t.languages, function (v, k) {
                    ...
                    if (v == t.selectedLang)t.selectedItem = mi;
                })
            });
            return c;
        }
        ...
        if (!m) {
            ...
            m = ed.controlManager.createDropMenu('spellcheckermenu', {offset_x:p1.x, offset_y:p1.y, 'class':'mceNoIcons'});
            t._menu = m;
        }
        if (dom.hasClass(e.target, 'mceItemHiddenSpellWord')) {
            ...
            t._sendRPC('getSuggestions', [t.selectedLang, dom.decode(e.target.innerHTML)], function (r) {
                ...
                if (r.length > 0) {
                    ...
                    m.addSeparator();
                } else m.add({title:'spellchecker.no_sug', 'class':'mceMenuItemTitle'}).setDisabled(1);
                m.add({title:'spellchecker.ignore_word', onclick:function () {
                    dom.remove(e.target, 1);
                    t._checkDone();
                }});
                m.add({title:'spellchecker.ignore_words', onclick:function () {
        ...
        JSONRequest.sendRPC({url:url, method:m, params:p, success:cb, error:function (e, x) {
            t.editor.setProgressState(0);
