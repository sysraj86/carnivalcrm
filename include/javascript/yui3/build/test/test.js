...
YUI.add('test', function (Y) {
    ...
    Y.Test.Case.prototype = {resume:function (segment) {
    ...
    Y.Test.Suite.prototype = {add:function (testObject) {
    ...
    Y.Test.Runner = (function () {
        ...
        function TestRunner() {
            ...
            this.masterSuite = new Y.Test.Suite("YUI Test Results");
            this._cur = null;
            this._root = null;
            this._log = true;
            this._waiting = false;
        ...
        Y.extend(TestRunner, Y.Event.Target, {TEST_CASE_BEGIN_EVENT:"testcasebegin", TEST_CASE_COMPLETE_EVENT:"testcasecomplete", TEST_SUITE_BEGIN_EVENT:"testsuitebegin", TEST_SUITE_COMPLETE_EVENT:"testsuitecomplete", TEST_PASS_EVENT:"pass", TEST_FAIL_EVENT:"fail", TEST_IGNORE_EVENT:"ignore", COMPLETE_EVENT:"complete", BEGIN_EVENT:"begin", disableLogging:function () {
            ...
            var node = parentNode.appendChild(testCase), prop, testName;
    ...
    Y.Assert = {_asserts:0, _formatMessage:function (customMessage, defaultMessage) {
    ...
    Y.extend(Y.Assert.Error, Error, {getMessage:function () {
        return this.message;
    }, toString:function () {
    ...
    Y.extend(Y.Assert.ComparisonFailure, Y.Assert.Error, {getMessage:function () {
    ...
    Y.extend(Y.Assert.UnexpectedValue, Y.Assert.Error, {getMessage:function () {
    ...
    Y.ArrayAssert = {contains:function (needle, haystack, message) {
        Y.Assert._increment();
    ...
    Y.DateAssert = {datesAreEqual:function (expected, actual, message) {
        Y.Assert._increment();
    ...
    Y.Test.Format.XML = function (results) {
        function serializeToXML(results) {
            var l = Y.Lang, xml = "<" + results.type + " name=\"" + xmlEscape(results.name) + "\"";
    ...
    Y.Test.Reporter.prototype = {constructor:Y.Test.Reporter, addField:function (name, value) {
    ...
    Y.Mock.expect = function (mock, expectation) {
        if (!mock.__expectations) {
            mock.__expectations = {};
        }
        if (expectation.method) {
            var name = expectation.method, args = expectation.args || expectation.arguments || [], result = expectation.returns, callCount = Y.Lang.isNumber(expectation.callCount) ? expectation.callCount : 1, error = expectation.error, run = expectation.run || function () {
            };
