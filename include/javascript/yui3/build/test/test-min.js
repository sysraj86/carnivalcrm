...
YUI.add("test", function (B) {
    ...
    B.Test.Case.prototype = {resume:function (C) {
    ...
    B.Test.Suite.prototype = {add:function (C) {
    ...
    B.Test.Runner = (function () {
        ...
        D.prototype = {appendChild:function (E) {
            var F = new D(E);
        ...
        function C() {
            ...
            this.masterSuite = new B.Test.Suite("YUI Test Results");
            this._cur = null;
            this._root = null;
            this._log = true;
            this._waiting = false;
            ...
            for (var E = 0; E < F.length; E++) {
                this.subscribe(F[E], this._logEvent, this, true);
            }
        }

        B.extend(C, B.Event.Target, {TEST_CASE_BEGIN_EVENT:"testcasebegin", TEST_CASE_COMPLETE_EVENT:"testcasecomplete", TEST_SUITE_BEGIN_EVENT:"testsuitebegin", TEST_SUITE_COMPLETE_EVENT:"testsuitecomplete", TEST_PASS_EVENT:"pass", TEST_FAIL_EVENT:"fail", TEST_IGNORE_EVENT:"ignore", COMPLETE_EVENT:"complete", BEGIN_EVENT:"begin", disableLogging:function () {
            this._log = false;
            ...
            if (B.Lang.isObject(E.testObject)) {
                ...
                if (E.testObject instanceof B.Test.Suite) {
                    ...
                    this.fire(this.TEST_SUITE_COMPLETE_EVENT, {testSuite:E.testObject, results:E.results});
                } else {
                    if (E.testObject instanceof B.Test.Case) {
                        this.fire(this.TEST_CASE_COMPLETE_EVENT, {testCase:E.testObject, results:E.results});
                    }
                }
            }
        }, _next:function () {
            ...
            if (F !== null) {
                ...
                if (B.Lang.isObject(E)) {
                    if (E instanceof B.Test.Suite) {
                        this.fire(this.TEST_SUITE_BEGIN_EVENT, {testSuite:E});
                        E.setUp();
                    } else {
                        if (E instanceof B.Test.Case) {
                            this.fire(this.TEST_CASE_BEGIN_EVENT, {testCase:E});
                        }
                    }
            ...
            var J = E.testObject;
            var G = E.parent.testObject;
            if (G.__yui_wait) {
                clearTimeout(G.__yui_wait);
                delete G.__yui_wait;
            }
            var M = (G._should.fail || {})[J];
            var F = (G._should.error || {})[J];
            var H = false;
            var K = null;
            try {
                I.apply(G);
                if (M) {
                    K = new B.Assert.ShouldFail();
                    H = true;
                } else {
                    if (F) {
                        K = new B.Assert.ShouldError();
                        H = true;
                    }
                }
            } catch (L) {
                ...
                if (L instanceof B.Assert.Error) {
                    if (!M) {
                        K = L;
                        H = true;
                    }
                } else {
                    if (L instanceof B.Test.Wait) {
                        if (B.Lang.isFunction(L.segment)) {
                            if (B.Lang.isNumber(L.delay)) {
                                if (typeof setTimeout != "undefined") {
                                    G.__yui_wait = setTimeout(function () {
                                        B.Test.Runner._resumeTest(L.segment);
                                    }, L.delay);
                                    this._waiting = true;
                                } else {
                        ...
                        if (!F) {
                            K = new B.Assert.UnexpectedError(L);
                            H = true;
                        } else {
                            if (B.Lang.isString(F)) {
                                ...
                                if (B.Lang.isFunction(F)) {
                                    ...
                                    if (B.Lang.isObject(F)) {
                                        if (!(L instanceof F.constructor) || L.message != F.message) {
                                            K = new B.Assert.UnexpectedError(L);
                                            H = true;
                                        }
                                    }
            ...
            if (H) {
                this.fire(this.TEST_FAIL_EVENT, {testCase:G, testName:J, error:K});
            } else {
                this.fire(this.TEST_PASS_EVENT, {testCase:G, testName:J});
            }
            G.tearDown();
            E.parent.results[J] = {result:H ? "fail" : "pass", message:K ? K.getMessage() : "Test passed", type:"test", name:J};
            if (H) {
            ...
            if (G) {
                H.parent.results[E] = {result:"ignore", message:"Test ignored", type:"test", name:E};
                H.parent.results.ignored++;
                H.parent.results.total++;
                this.fire(this.TEST_IGNORE_EVENT, {testCase:F, testName:E});
                if (typeof setTimeout != "undefined") {
                    setTimeout(function () {
                        B.Test.Runner._run();
                    }, 0);
            ...
            this._resumeTest(E || function () {
            });
        }, run:function (E) {
            var F = B.Test.Runner;
            F._buildTestTree();
            F._root.results.duration = (new Date()).valueOf();
            F.fire(F.BEGIN_EVENT);
            F._run();
        }});
        return new C();
    })();
    B.Assert = {_asserts:0, _formatMessage:function (D, C) {
        var E = D;
        if (B.Lang.isString(D) && D.length > 0) {
            return B.Lang.substitute(D, {message:C});
        } else {
            return C;
        }
    }, _getCount:function () {
    ...
    B.extend(B.Assert.Error, Error, {getMessage:function () {
        return this.message;
    }, toString:function () {
    ...
    B.extend(B.Assert.ComparisonFailure, B.Assert.Error, {getMessage:function () {
    ...
    B.extend(B.Assert.UnexpectedValue, B.Assert.Error, {getMessage:function () {
    ...
    B.ArrayAssert = {contains:function (E, D, C) {
        B.Assert._increment();
    ...
    B.ObjectAssert = {areEqual:function (D, E, C) {
        B.Assert._increment();
    ...
    B.DateAssert = {datesAreEqual:function (D, F, C) {
        B.Assert._increment();
    ...
    B.Test.Format.XML = function (E) {
        var C = B.Lang;
        var D = "<" + E.type + ' name="' + A(E.name) + '"';
        if (E.type == "test") {
            D += ' result="' + A(E.result) + '" message="' + A(E.message) + '">';
        } else {
            D += ' passed="' + E.passed + '" failed="' + E.failed + '" ignored="' + E.ignored + '" total="' + E.total + '">';
            B.Object.each(E, function (F, G) {
                if (C.isObject(F) && !C.isArray(F)) {
                    D += arguments.callee(F);
                }
            });
        }
        D += "</" + E.type + ">";
    ...
    B.Test.Format.XML = function (D) {
        function C(G) {
            var E = B.Lang, F = "<" + G.type + ' name="' + A(G.name) + '"';
        ...
        return'<?xml version="1.0" charset="UTF-8"?>' + C(D);
    };
    B.Test.Format.JUnitXML = function (C) {
        function D(G) {
            ...
            switch (G.type) {
                case"test":
                    ...
                    break;
                case"testcase":
                    F = '<testsuite name="' + A(G.name) + '" tests="' + G.total + '" failures="' + G.failed + '">';
                    B.Object.each(G, function (I, J) {
                        if (E.isObject(I) && !E.isArray(I)) {
                            F += D(I);
                        }
                    });
                    F += "</testsuite>";
        ...
        return'<?xml version="1.0" charset="UTF-8"?>' + D(C);
    };
    B.namespace("Test");
    ...
    B.Test.Reporter.prototype = {constructor:B.Test.Reporter, addField:function (C, D) {
        this._fields[C] = D;
    ...
    B.Mock.expect = function (D, H) {
        ...
        if (H.method) {
            var G = H.method, F = H.args || H.arguments || [], C = H.returns, J = B.Lang.isNumber(H.callCount) ? H.callCount : 1, E = H.error, I = H.run || function () {
            };
