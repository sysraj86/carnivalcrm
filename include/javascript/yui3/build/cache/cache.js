...
YUI.add('cache', function (Y) {
    var LANG = Y.Lang, Cache = function () {
        Cache.superclass.constructor.apply(this, arguments);
    };
    Y.mix(Cache, {NS:"cache", NAME:"cache", ATTRS:{max:{value:0, validator:function (value) {
    ...
    }}, uniqueKeys:{value:false, validator:function (value) {
        return(LANG.isBoolean(value));
    }}, entries:{readOnly:true, getter:function () {
        return this._entries;
    }}}});
    Y.extend(Cache, Y.Plugin.Base, {_entries:null, initializer:function (config) {
        ...
        var entries = this._entries, max = this.get("max"), entry = e.entry;
        ...
        if (this.get("entries") && (this.get("max") > 0) && (LANG.isValue(request) || LANG.isNull(request) || LANG.isUndefined(request))) {
        ...
        var entries = this._entries, length = entries.length, entry = null, i = length - 1;
