...
YUI.add("node-focusmanager", function (B) {
    var J = "activeDescendant", L = "id", I = "disabled", N = "tabIndex", E = "focused", A = "focusClass", Q = "circular", C = "UI", F = "key", G = J + "Change", O = "host", P = {37:true, 38:true, 39:true, 40:true}, M = {"a":true, "button":true, "input":true, "object":true}, H = B.Lang, K = B.UA, D = function () {
        D.superclass.constructor.apply(this, arguments);
    };
    D.ATTRS = {focused:{value:false, readOnly:true}, descendants:{getter:function (R) {
        return this.get(O).all(R);
    }}, activeDescendant:{setter:function (V) {
        var T = H.isNumber, S = B.Attribute.INVALID_VALUE, R = this._descendantsMap, Y = this._descendants, X, U, W;
        if (T(V)) {
            X = V;
            U = X;
        } else {
    ...
    B.extend(D, B.Plugin.Base, {_stopped:true, _descendants:null, _descendantsMap:null, _focusedNode:null, _lastNodeIndex:0, _eventHandlers:null, _initDescendants:function () {
        var Y = this.get("descendants"), R = {}, W = -1, V, U = this.get(J), X, S, T = 0;
        if (H.isUndefined(U)) {
            U = -1;
        }
        if (Y) {
        ...
        var S = this._focusedNode, T = this.get(A), R;
        if (T) {
        ...
        var S = this._prevKeyHandler, R = this._nextKeyHandler;
        if (S) {
            S.detach();
        }
        if (R) {
        ...
        var R = S.target, T = R.get("nodeName").toLowerCase();
        if (S.keyCode === 13 && (!M[T] || (T === "a" && !R.getAttribute("href")))) {
            R.simulate("click");
        }
    }, _attachKeyHandler:function () {
        this._detachKeyHandler();
        var U = this.get("keys.next"), S = this.get("keys.previous"), T = this.get(O), R = this._eventHandlers;
        if (S) {
        ...
        var W = this.get(O), R = U.target, V = W.contains(R), T, S = function (Y) {
            var X = false;
            if (!Y.compareTo(W)) {
        ...
        var U = this._focusTarget || W.target, S = this.get(E), V = this.get(A), T = this._focusedNode, R;
