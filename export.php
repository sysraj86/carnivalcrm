...
$html = '';
foreach($content as $val_cont){
    $digit = "/^\/'".$value."$/";
    $temp = explode("\"".getDelimiter()."\"",$val_cont);
    $html .= '<tr>';
    $pattern =  "/^([0-9]{2})\/([0-9]{2})\/([0-9]{4}) ([01][0-9]|2[0-3]):([0-5][0-9]) .m$/";
...
$template = file_get_contents("custom/ExportTemplate/Report_CUS_files/sheet001.tpl");
...
header("Content-disposition: xls; filename={$filename}.xls; size=".strlen($template));
echo $template;
exit();   
