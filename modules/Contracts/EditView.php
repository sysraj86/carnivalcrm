<?php
    ...
    if(isset($_REQUEST['record'])) {
        ...
        echo "\n<p>\n";
        echo get_module_title($mod_strings['LBL_MODULE_ID'], $mod_strings['LBL_MODULE_NAME'].": ".$focus->name." : ".$focus->type, true);
        echo "\n</p>\n";
        ...
        $GLOBALS['log']->info("Contract detail view");   
        ...
        $ss->assign("DATE", get_select_options($app_list_strings['date_options_dom'],$focus->date));
        $ss->assign("MONTH", get_select_options($app_list_strings['month_options_dom'],$focus->month));
        $ss->assign("YEAR", $focus->year);
        $ss->assign("DATE_OF_CONTRACTS", $focus->date_of_contracts);
        ...
        $ss->assign("PHONE_A", $focus->phone_a);
        $ss->assign("FAXA", $focus->fax_a);
        $ss->assign("FAXB", $focus->fax_b);
        ...
        $ss->assign("TOUR_NAME",  $focus->groupprogracontracts_name);   
        $ss->assign("TOUR_ID",  $focus->groupprogr4251rograms_ida);   
        ...
        /*   $ss->assign("DOTUOI",  $focus->dotuoi);   
        $ss->assign("TONG_SL_KHACH",  $focus->tong_sl_khach);   
        $ss->assign("GIA_TOUR",  $focus->gia_tour);   
        $ss->assign("THUE",  $focus->thue);   
        $ss->assign("THANHTIEN",  $focus->thanhtien);*/  
        ...
        $ss->assign("CURRENCY",   get_select_options_with_id($app_list_strings['currency_dom'], $focus->tiente)); 
        ...
        $ss->assign("CONTRACT_CONDITIONS", $focus->get_contract_condition_editview_line());
        ...
        $ss->assign("TIENHUYPHAT", $focus->tienhuyphat);  
        $ss->assign("SOHOCHIEUKHACHLE", $focus->sohochieukhachle);  
        $ss->assign("NGAYCAPHOCHIEU", $focus->ngaycaphochieu);  
        $ss->assign("TIENTEHUYPHAT", get_select_options_with_id($app_list_strings['currency_dom'], $focus->tientehuyphat));  
        ...
        if($_REQUEST['record'] != ''){
            if($focus->type == ''){
             ...
             $ss->display("modules/Contracts/tpls/{$focus->type}.tpl") ; 
        }
            ...
            if($type == '')
                $type = 'home';
            $ss->assign('TYPE',$type);
            $ss->display("modules/Contracts/tpls/{$type}.tpl");
        }    
