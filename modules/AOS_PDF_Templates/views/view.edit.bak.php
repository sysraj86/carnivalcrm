...
class AOS_PDF_TemplatesViewEdit extends ViewEdit {
	...
	function setFields(){
        ...
        foreach($travelguide->field_defs as $name => $arr){
            if(!((isset($arr['dbType'])&& strtolower($arr['dbType'])=='id')||$arr['type']=='id' || $arr['type']== 'link')){
                if($arr['vname'] !='LBL_DELETED'){
                    $travelguide_options_array ['$travelguide_'.$name] = translate($arr['name'],'TravelGuides');
                }
            }
        }
        ...
        $hotel = new Hotels();
         foreach($hotel->field_defs as $name => $arr){
            if(!((isset($arr['dbType'])&& strtolower($arr['dbType'])=='id')||$arr['type']=='id' || $arr['type']== 'link')){
                if($arr['vname'] !='LBL_DELETED'){
                    $hotel_options_array ['$hotel_'.$name] = translate($arr['name'],'Hotels');
                }
            }
        }        
        // restaurant
        $res = new Restaurants();
         foreach($res->field_defs as $name => $arr){
            if(!((isset($arr['dbType'])&& strtolower($arr['dbType'])=='id')||$arr['type']=='id' || $arr['type']== 'link')){
                if($arr['vname'] !='LBL_DELETED'){
                    $res_options_array ['$res_'.$name] = translate($arr['name'],'Restaurants');
                }
            }
        }
        ...
        foreach($fit->field_defs as $name => $arr){
            if(!((isset($arr['dbType'])&& strtolower($arr['dbType'])=='id')||$arr['type']=='id' || $arr['type']== 'link')){
                if($arr['vname'] !='LBL_DELETED'){
                    $fit_options_array ['$fit_'.$name] = translate($arr['name'],'FITs');
                }
            }
        }
        ...
        foreach($contracts->field_defs as $name => $arr){
            if(!((isset($arr['dbType'])&& strtolower($arr['dbType'])=='id')||$arr['type']=='id' || $arr['type']== 'link')){
                if($arr['vname'] !='LBL_DELETED'){
                    $contracts_options_array ['$contracts_'.$name] = translate($arr['name'],'Contracts');
                }
            }
        }
