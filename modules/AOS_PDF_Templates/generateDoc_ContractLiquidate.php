...
<?php
/*******************************Mô Tả******************************
 1. Chức năng chính của file gồm :
 - Dùng để chuyển dữ liệu từ màn hình detail(khi mình nhấp export) ra file word.
 - Và chuyển dữ liệu từ các module phụ (Các module value,phát sinh tăng/giảm..) ra file word.
 
 2. Nhận dữ liệu:
 - Nhận trực tiếp từ file view.detail.php  (Nhận id và module name)
 
 3. Xuất dữ liệu:
 - Xuất ra một file word đã được format.
 
 4. Xử lý:
 - Lấy dữ liệu(id và module name) từ file view.detail.php
 - Tạo đối tượng module (cần xuất dữ liệu), lấy dữ liệu cho module thông qua id .
 - Tạo đối tượng template ,lấy dữ liệu template từ thuộc tính(thông qua một 
   dropdow_template) teamplate của module.
 - Lấy dữ liệu của template này đưa vào một biến text.
 - Chuyển dữ liệu từ module và biến text này vào hàm 'templateParser::parse_template', hàm
   này trả ra một biến text đã được điền dữ liệu.
 - Sau đó xuất ra file word với nội dung là đoạn text này và các formmat cần thiết.
...
*******************************************************************/
    ...
    $contractliquidatedecre = array();
    $sql2 = "SELECT * FROM contractliquidatedecre WHERE contract_liquidate_id='".$this->id."' AND deleted =0";
    $res2 = $module->db->query($sql2);
    while($row2 = $module->db->fetchByAssoc($res2)){
        $contractliquidatedecre[$row2['id']] = $row2['contract_liquidate_id'];
    }
    ...
    // Lấy dữ liệu vào mảng $object_arr để đổ vào template.
    $object_arr = array();
    $object_arr[$module_type] = $module->id;
    $object_arr['Contracts']  = $contract->id;
    $object_arr['Tours']  = $tour->id;
    ...
    $contractliquidate_value = new ContractLiquidateValues();
    foreach($contractliquidate_value->field_defs as $name => $arr){
        if(!((isset($arr['dbType']) && strtolower($arr['dbType']) == 'id') || $arr['type'] == 'id' || $arr['type'] == 'link')){
            $curNum = strpos($text,'$contractliquidatevalues_'.$name);
            if($curNum){
                if($curNum < $firstNum || $firstNum == 0){
                    $firstValue = '$contractliquidatevalues_'.$name;
                    $firstNum = $curNum;
                }
                else if($curNum > $lastNum){
                        $lastValue = '$contractliquidatevalues_'.$name;
                        $lastNum = $curNum;
                    }
            }
        }
    ...
    if($firstValue !== '' && $lastvalue !== ''){
        ...
        if(count($contractliquidatevalue) != 0){
            ...
            foreach($contractliquidatevalue as $id => $contractliquidateId){
                $obb['ContractLiquidateValues'] = $id;
                $obb['ContractLiquidate'] = $contractliquidateId;
    ...
    $contractliquidate_incre = new ContractLiquidateIncre();
    foreach($contractliquidate_incre->field_defs as $name => $arr){
        if(!((isset($arr['dbType']) && strtolower($arr['dbType']) == 'id') || $arr['type'] == 'id' || $arr['type'] == 'link')){
            $curNum = strpos($text,'$contractliquidateincre_'.$name);
            if($curNum){
                if($curNum < $firstNum || $firstNum == 0){
                    $firstValue = '$contractliquidateincre_'.$name;
                    $firstNum = $curNum;
                }
                else if($curNum > $lastNum){
                        $lastValue = $name;
    ...
    if($firstValue !== '' && $lastvalue !== ''){
        ...
        $parts = explode($lastValue,$parts[1]);
        $linePart = $firstValue . $parts[0] . $lastValue;
        if(count($contractliquidateincre) != 0){
            //Read line start <tr> value
            $tcount = strrpos($text,"<tr");
            $lsValue = substr($text,$tcount);
            $tcount=strpos($lsValue,">")+1;
            ...
            $sep = '';
            $tdTemp = explode($lsValue,$text);
            foreach($contractliquidateincre as $id => $contractliquidateId){
                $obb['ContractLiquidateIncre'] = $id;
                $obb['ContractLiquidate'] = $contractliquidateId;
                $text .= $sep . templateParser::parse_template($linePart, $obb);
                $sep = $leValue. $lsValue . $tdTemp[count($tdTemp)-1];
            }
    ...
    $contractliquidate_decre = new ContractLiquidateDecre();
    foreach($contractliquidate_decre->field_defs as $name => $arr){
        if(!((isset($arr['dbType']) && strtolower($arr['dbType']) == 'id') || $arr['type'] == 'id' || $arr['type'] == 'link')){
            $curNum = strpos($text,'$contractliquidatedecre_'.$name);
            if($curNum){
                if($curNum < $firstNum || $firstNum == 0){
                    $firstValue = '$contractliquidatedecre_'.$name;
                    $firstNum = $curNum;
                }
                else if($curNum > $lastNum){
                        $lastValue = '$contractliquidatedecre_'.$name;
                        $lastNum = $curNum;
                    }
            }
        }
    ...
    if($firstValue !== '' && $lastvalue !== ''){
        ...
        $parts = explode($lastValue,$parts[1]);
        $linePart = $firstValue . $parts[0] . $lastValue;
        if(count($contractliquidatedecre) != 0){
            //Read line start <tr> value
            $tcount = strrpos($text,"<tr");
            $lsValue = substr($text,$tcount);
            $tcount=strpos($lsValue,">")+1;
            ...
            $sep = '';
            $tdTemp = explode($lsValue,$text);
            foreach($contractliquidatedecre as $id => $contractliquidateId){
                $obb['ContractLiquidateDecre'] = $id;
                $obb['ContractLiquidate'] = $contractliquidateId;
                $text .= $sep . templateParser::parse_template($linePart, $obb);
                $sep = $leValue. $lsValue . $tdTemp[count($tdTemp)-1];
            }
...
ul
    ...
    $file_name = "Contract_Liquidate_".$module->number.".doc";
