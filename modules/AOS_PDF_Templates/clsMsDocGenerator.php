...
class clsMsDocGenerator{
     ...
     * @param $sessionName: Word 2003: Section1, Word 2010: WordSection1
     * @param $header_link: has header or not? 
     * @param $footer_link: has header or not? 
	 ...
	 */
	function newSession($sessionName="WordSection1",$header_link="", $footer_link="", $pageOrientation = NULL, $pageType = NULL, $topMargin = NULL, $rightMargin = NULL, $bottomMargin = NULL, $leftMargin = NULL){
		...
		/*$this->formatBuffer .= " @page
                {mso-footnote-separator:url('{$header_link}') fs;
                mso-footnote-continuation-separator:url('{$header_link}') fcs;
                mso-endnote-separator:url('{$header_link}') es;
                mso-endnote-continuation-separator:url('{$header_link}') ecs;}";
        ...
        if($header_link)
            $this->formatBuffer .= " mso-header:url('{$header_link}') h1\n";
        if($footer_link)
            $this->formatBuffer .= "  mso-footer:url('{$footer_link}') f1;\n";
	...
	function output($fileName = '', $saveInPath = ''){
        ...
        $outputCode.= '<html xmlns:v="urn:schemas-microsoft-com:vml"
		...
		if($saveInPath == ''){
            ...
            if (!isset($_SERVER['HTTP_ACCEPT_ENCODING']) OR empty($_SERVER['HTTP_ACCEPT_ENCODING'])) {
                ...
                header('Content-Length: '.strlen($this->documentBuffer));
            }
            header('Content-disposition: attachment; filename="'.$fileName.'"');
             //echo $this->buffer;
            echo $outputCode;    
    ...
    /**
	 * public void setCSSFile(string $path_to_file)
	 * @param $path_to_file: path to css file
	 */
	function setCSSFile($path_to_file){
		$this->cssFile = $path_to_file;
	}//end setCSSFile()
	...
	function prepareDefaultHeader(){
          ...
          /**
          * Added by SugarCRM VN
          * 
          * @var clsMsDocGenerator
          */
          
            $this->formatBuffer .= 'p.MsoNormal, li.MsoNormal, div.MsoNormal
            {mso-style-unhide:no;
            ...
            mso-fareast-font-family:'.$this->fontFamily.';
